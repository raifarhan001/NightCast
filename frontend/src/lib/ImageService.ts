export const ImageService = {
  ...
  FallbackImage(type: 'poster' | 'backdrop' | 'profile', title?: string): string {
    const displayTitle = title || 'Vidking';
    ...
    const svg = `
      <svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
        ...
        <text x="50%" y="${type === 'poster' ? '58%' : '56%'}" dominant-baseline="middle" text-anchor="middle" font-family="Inter, sans-serif" font-size="${type === 'poster' ? 10 : 13}" font-weight="bold" fill="#00D2FF" opacity="0.25" letter-spacing="4">
